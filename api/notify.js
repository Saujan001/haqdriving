...
export default async function handler(req, res) {
  ...
  const OWNER_EMAIL = process.env.OWNER_EMAIL || 'info@haqdriving.com.au';
  const FROM_ADDRESS = 'Haq Driving <noreply@haqdriving.com.au>';
...
<html><body style="font-family:Inter,sans-serif;color:#0F1B2A;max-width:600px;margin:0 auto;padding:24px">
  <div style="background:#1a3a5c;border-radius:12px;padding:24px 28px;margin-bottom:24px">
    <div style="font-size:22px;font-weight:800;color:#fff;letter-spacing:-0.02em">Haq Driving</div>
  ...
  <p style="color:#6B7585;font-size:12px;margin-top:24px">This enquiry was submitted via the Haq Driving website.</p>
</body></html>`;

  // Customer confirmation
  const firstName = escHtml(full_name.split(' ')[0]);
  const customerHtml = `
<!DOCTYPE html>
<html><body style="font-family:Inter,sans-serif;color:#0F1B2A;max-width:600px;margin:0 auto;padding:24px">
  <div style="background:#1a3a5c;border-radius:12px;padding:24px 28px;margin-bottom:24px">
    <div style="font-size:22px;font-weight:800;color:#fff;letter-spacing:-0.02em">Haq Driving</div>
    <div style="font-size:13px;color:#93A8C2;margin-top:4px">Driving School · Sydney NSW</div>
  </div>
  <h2 style="font-size:22px;font-weight:800;margin-bottom:10px">Thanks, ${firstName}! 👋</h2>
  ...
  <div style="background:#EFF5FF;border-radius:12px;padding:20px 24px;margin-bottom:24px">
    <div style="font-weight:700;font-size:15px;margin-bottom:12px;color:#1a3a5c">Contact us directly</div>
    <div style="font-size:14px;color:#475467;line-height:2">
      📞 <a href="tel:+61450000000" style="color:#2563EB;font-weight:600">0450 000 000</a><br>
      ✉️ <a href="mailto:info@haqdriving.com.au" style="color:#2563EB;font-weight:600">info@haqdriving.com.au</a><br>
  ...
  <p style="color:#6B7585;font-size:13px;margin-top:8px"><strong>Haq Driving School</strong><br>Sydney NSW</p>
</body></html>`;

  try {
    await Promise.all([
      sendEmail(OWNER_EMAIL, `New enquiry from ${full_name} — ${lesson_type || 'Driving Lesson'}`, ownerHtml),
      sendEmail(email, 'We received your enquiry — Haq Driving', customerHtml),
    ]);
